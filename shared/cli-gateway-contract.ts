...
const channelSubscribeFrameSchema: RelayJsonSchema = {
  ...
  oneOf: [
    ...
    {
      title: 'ChannelsSubscribeResumedFrameV1',
      type: 'object',
      additionalProperties: false,
      properties: {
        schemaVersion: { const: 1 },
...
const commandSpecs: readonly RelayCliGatewayCommandSpec[] = [
  ...
  },
  {
    ...
    summary:
      'List native provider sessions (claude/codex/pi/prime-agent/dsh/cursor/antigravity) from local provider stores with per-provider install status.',
    stable: true,
    transport: 'hub-http',
    requiresAuth: true,
    capabilityHints: ['session:read'],
